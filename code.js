...
const onGainFocusID = RPM.Manager.Plugins.getParameter(pluginName, "Gain focus event ID");
const onLoseFocusID = RPM.Manager.Plugins.getParameter(pluginName, "Lose focus event ID");
...
function raycast(v, dist, ignore = false)
{
    ...
    return RPM.Core.Game.current.variables[v];
}
...
{
    RPM.Manager.Events.sendEventDetection(null, -1, false, onGainFocusID, [null]);
});

window.addEventListener("blur", (e) =>
{
    RPM.Manager.Events.sendEventDetection(null, -1, false, onLoseFocusID, [null]);
});

RPM.Manager.Plugins.registerCommand(pluginName, "Get object under cursor", (variableID, x, y, ignoreHero) =>
{
    x = RPM.Common.ScreenResolution.getScreenXReverse(x);
    y = RPM.Common.ScreenResolution.getScreenYReverse(y);
    if (RPM.Manager.Stack.top instanceof RPM.Scene.Map && !RPM.Scene.Map.current.loading)
    {
        ...
        RPM.Core.Game.current.variables[variableID] = raycast(variableID, -1, ignoreHero);
    }
});
...
RPM.Manager.Plugins.registerCommand(pluginName, "Raycast", (Ax, Ay, Az, Bx, By, Bz, variableID) =>
{
    if (RPM.Manager.Stack.top instanceof RPM.Scene.Map && !RPM.Scene.Map.current.loading)
    {
        ...
        vb.sub(va).normalize();
        raycaster.set(va, vb);
        RPM.Core.Game.current.variables[variableID] = raycast(variableID, dist);
    }
});
